...
export const useMedicineStore = defineStore('medicine', () => {
  ...
  const fetchMedicines = async () => {
    ...
    error.value = null
    try {
      // Fetch all medicines (using a large limit for now to keep frontend filtering working as before)
      // In a real app, we would implement server-side filtering/pagination in the public view too
      const [medicinesList, safetyData] = await Promise.all([
        MedicineService.getAllMedicines(), 
        MedicineService.getSafetyInfo()
      ])
      
      medicines.value = medicinesList
      originalMedicines.value = [...medicinesList] // Deep copy to avoid reference issues
      
      // Handle safety info structure mismatch if necessary
      // Backend returns { caveat: [], remark: [], guide: [], delivery: [] }
      // Store expects same.
